...
| — | `--no-client-results` | Refuses `return_results` in the handshake: results reach only the SBS output and CSVs. By default a result goes to the receivers that heard its message, as in mlat-server, and only those within 600 km of the fix. |
